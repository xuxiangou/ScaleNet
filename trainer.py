    ...
    def convert_cgcnn_format(batch):
        ...
        for i, ((total_atom_fea, total_node_list, total_nbr_adj_node, total_nbr_dist_node, total_nbr_bond_cat,
            ...
            for ads_iter in range(0, num_ads):
                ...
                ads_atom_idx.extend([ads_iter + base_idx] * len(
                    total_node_list[ads_iter]))  # Labelling each ads in every crystal entry wrt total configs
                base_idx += n_i
...
class Trainer():
    ...
    def main(self, model_cnf_dir):
        # initiate parameters through yaml
        yaml = YAML(typ="safe")
        ...
        # with torch.no_grad():
        #     dataset.set_label((dataset.get_label - dataset.get_mean) / dataset.get_std)
        ...
        if train_params["mode"]:
            ...
            # plot all the metric in the training process
            plot_model_metric(train_loss=train_loss_list,
                              val_mae=valid_mae_list,
                              save_dir=model_save_dir,
                              task=model_params["targets"][0])
        logger.info("Testing")
