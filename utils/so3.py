...
def sh_indices(lmax: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    midx = torch.cat([torch.arange(-l_id, l_id + 1) for l_id in ls])
    return lidx, midx
...
def generate_clebsch_gordan_rsh(lmax: int, parity_invariance: bool = True) -> torch.Tensor:
    ...
    # cast to real
    cg_rsh = cg_rsh.real.to(torch.float32)
    return cg_rsh
...
def round_cmp(x: torch.Tensor, decimals: int = 1):
    ...
    Returns:
        Resulting rounded tensors including real and imaginary parts.
    """
    return torch.round(x.real, decimals=decimals) + 1j * torch.round(x.imag, decimals=decimals)
