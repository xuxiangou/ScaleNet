"""
This file comes from AugLiChem modified by xiangou
url: https://github.com/BaratiLab/AugLiChem?tab=readme-ov-file
"""
...
class RotationTransformation(AbstractTransformation):
    ...
    def apply_transformation(self, structure, axis=None, angle=None, perturb=None,
                             angle_in_radians=False, seed=42):
        """
        ...
        s.apply_operation(self._symmop, fractional=False)
        # s = self._symmop(s)
        return s
...
class PerturbStructureTransformation(AbstractTransformation):
    ...
    def __init__(
            self,
            distance: float = 0.08,
            min_distance: Optional[Union[None, float]] = 0.01,
    ):
        """
        Args:
    ...
    def perturb(self, structure: Structure, distance: float, min_distance: float or None = None) -> None:
        ...
        for idx in range(len(structure._sites)):
            self.translate_sites(structure, [idx], get_rand_vec(), frac_coords=False, to_unit_cell=False)

    def translate_sites(
            self, structure: Structure, indices: int or Sequence[int], vector: ArrayLike, frac_coords: bool = True,
        ...
        for idx in indices:
            ...
            if frac_coords:
                ...
                fcoords = structure._lattice.get_fractional_coords(site.coords + vector)
            if to_unit_cell:
                fcoords = [np.mod(f, 1) if p else f for p, f in zip(structure.lattice.pbc, fcoords)]
            structure[idx].frac_coords = fcoords

    def apply_transformation(self, structure: Structure, seed=42) -> Structure:
        """
        Apply the transformation.
        Args:
        ...
        # This method can change the structure
        self.perturb(s, self.distance, min_distance=self.min_distance)
        return s
...
class SwapAxesTransformation(object):
    """
    Swap coordinate axes in the crystal structure.
    """
    ...
    def apply_transformation(self, crys, seed=42, _test_choice=None):
...
class RemoveSitesTransformation(AbstractTransformation):
    ...
    def apply_transformation(self, structure, seed=42):
        """
        Apply the transformation.
        Arg:
...
class TranslateSitesTransformation(AbstractTransformation):
    ...
    def __init__(self, indices_to_move=None, translation_vector=None, vector_in_frac_coords=False):
        """
        Args:
    ...
    def apply_transformation(self,
                             ...
                             vector_in_frac_coords=None, seed=42
                             ):
        """
        Apply the transformation.
        Arg:
...
class CubicSupercellTransformation(AbstractTransformation):
    ...
    def apply_transformation(self, structure: Structure, seed=42, perturb=None) -> Structure:
        """
