...
class PourbaixEntry(MSONable, Stringify):
    ...
    def energy(self):
        ...
        return self.uncorrected_energy + self.conc_term
...
class PourbaixPlotter:
    ...
    def get_pourbaix_plot(
        ...
        max_adsorbate = max([entry.composition.get("O", 0) for entry in self._pbx._filtered_entries])
        last_entry_MLs = []
        for entry, vertices in self._pbx._stable_domain_vertices.items():
            ML = entry.composition.get("O", 0) / max_adsorbate
            whether_throw = False
            # for last_ml in last_entry_MLs:
            #     if abs(ML - last_ml) < 0.1:
            #         whether_throw = True
            #         break
            # if whether_throw:
            #     continue
            last_entry_MLs.append(ML)
        ...
        ax.set_xlabel(xlabel="pH").set_fontsize(fontsize=label_fontsize)
        ax.set_ylabel(ylabel="E (V)").set_fontsize(fontsize=label_fontsize)
