...
def compute_pair_vector_and_distance(g: dgl.DGLGraph):
    ...
    """
    dst_pos = g.ndata["pos"][g.edges()[1].long()] + g.edata["pbc_offshift"]
    src_pos = g.ndata["pos"][g.edges()[0].long()]
...
def _compute_3body(g: dgl.DGLGraph):
    ...
    triple_bond_indices = np.empty((n_triple, 2), dtype=np.int32)
    ...
    n_triple_ij = torch.tensor(n_triple_ij, dtype=torch.int32, device=g.device)
...
def _create_directed_line_graph(graph: dgl.DGLGraph, threebody_cutoff: float) -> dgl.DGLGraph:
    ...
    with torch.no_grad():
        ...
        lg_src = torch.empty(num_edges_per_bond.sum(), dtype=torch.int32, device=graph.device)
        lg_dst = torch.empty(num_edges_per_bond.sum(), dtype=torch.int32, device=graph.device)
