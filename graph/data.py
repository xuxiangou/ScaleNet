...
from graph.compute import compute_pair_vector_and_distance, create_line_graph
...
def collate_fn_graph(batch, include_line_graph: bool = False, multiple_values_per_target: bool = False):
    ...
    labels = (
        ...
        else torch.tensor([next(iter(d.values())) for d in labels], dtype=torch.float)
    )
...
def collate_fn_pes(batch, include_stress: bool = True, include_line_graph: bool = False, include_magmom: bool = False):
    ...
    s = (
        ...
        else torch.tensor(np.zeros(e.size(dim=0)), dtype=torch.float32)
    )
    m = (
...
class MGLDataset(DGLDataset):
    ...
    def process(self):
        ...
        if self.graph_labels is not None:
            ...
            state_attrs = torch.tensor(np.array(state_attrs), dtype=torch.float32)
    ...
    def __getitem__(self, idx: int):
        ...
        items = [
            ...
            self.lattices[idx],
            self.state_attr[idx],
            {k: torch.tensor(v[idx], dtype=torch.float32) for k, v in self.labels.items()},
        ]
        if self.include_line_graph:
