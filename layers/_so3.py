...
from utils.maths import binom, scatter_add
from utils.so3 import generate_clebsch_gordan_rsh, sh_indices, sparsify_clebsch_gordon
...
class RealSphericalHarmonics(nn.Module):
    ...
    def _generate_Ylm_coefficients(self, lmax: int):
        ...
        m = torch.arange(1, lmax + 1, dtype=torch.float32)[:, None]
        p = torch.arange(0, lmax + 1, dtype=torch.float32)[None, :]
        mask = p <= m
        mCp = binom(m, p)
        ...
        l_id = torch.arange(0, lmax + 1, dtype=torch.float32)[:, None, None]
        m = torch.arange(0, lmax + 1, dtype=torch.float32)[None, :, None]
        k = torch.arange(0, lmax // 2 + 1, dtype=torch.float32)[None, None, :]
