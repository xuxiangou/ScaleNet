...
from graph.compute import (
    compute_pair_vector_and_distance,
)
from layers._core import MLP
from layers._activations import ActivationFunction
from layers._bond import BondExpansion
from layers._readout import (
    ReduceReadOut,
    Set2SetReadOut,
    WeightedAtomReadOut,
    WeightedReadOut,
)
from layers._embedding import TensorEmbedding
from layers._graph_convolution import TensorNetInteraction
from utils.io import IOMixIn
from utils.maths import decompose_tensor, tensor_norm

if TYPE_CHECKING:
    from graph.converters import GraphConverter
...
class TensorNet(nn.Module, IOMixIn):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if is_intensive:
            ...
            dims_final_layer = [readout_feats, units, units, units]
    ...
    def forward(self, g: dgl.DGLGraph, state_attr: torch.Tensor | None = None, **kwargs):
        ...
        g.ndata["node_feat"] = x
        # split the adsorbate nodes and nanoparticle nodes
        g_list = dgl.unbatch(g)
        adsorbate_subgraph = []
        nanoparticle_subgraph = []
        for g in g_list:
            node_type = g.ndata["node_type"].tolist()
            is_adsorbate =  [i for i, value in enumerate(node_type) if value == 0 or value == 1]
            is_nanoparticle = [i for i, value in enumerate(node_type) if value == 2]
            adsorbate_subgraph.append(dgl.node_subgraph(g, is_adsorbate))
            nanoparticle_subgraph.append(dgl.node_subgraph(g, is_nanoparticle))
        adsorbate_subgraph = dgl.batch(adsorbate_subgraph)
        nanoparticle_subgraph = dgl.batch(nanoparticle_subgraph)
        ##############################
        if self.is_intensive:
            # node_vec = self.readout(g)
            # vec = node_vec  # type: ignore
            # output = self.final_layer(vec)
            adsorbate_node_vec = self.readout(adsorbate_subgraph)
            nanoparticle_node_vec = self.readout(nanoparticle_subgraph)
            adsorbate_output = self.final_layer(adsorbate_node_vec)
            nanoparticle_output = self.final_layer(nanoparticle_node_vec)
            if self.task_type == "classification":
                raise NotImplementedError("Classification task is not implemented yet.")
            return adsorbate_output, nanoparticle_output
        else:
            raise NotImplementedError("Extensive property prediction is not implemented yet.")
    ...
    def predict_structure(
        ...
        if graph_converter is None:
            from .ext.pymatgen import Structure2Graph
