...
from layers._atom_ref import AtomRef
from layers._zbl import NuclearRepulsion
...
class Potential(nn.Module, IOMixIn):
    ...
    def __init__(
        ...
        if element_refs is not None:
            self.element_refs = AtomRef(property_offset=torch.tensor(element_refs, dtype=torch.float32))
        else:
            self.element_refs = None
        # for backward compatibility
        if data_mean is None:
            data_mean = 0.0
        self.register_buffer("data_mean", torch.tensor(data_mean, dtype=torch.float32))
        self.register_buffer("data_std", torch.tensor(data_std, dtype=torch.float32))
    ...
    def forward(
        ...
        if self.calc_stresses:
            volume = (
                ...
                if torch.float32 == torch.float16
